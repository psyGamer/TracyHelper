...
public static class Profiler {
    ...
    /// <summary>
    /// Begins a new <see cref="Zone"/> and returns the handle to that zone.
    /// Time spent inside a zone is calculated by Tracy and shown in the profiler.
    /// A zone is ended when <see cref="Zone.Dispose"/> is called either automatically via disposal scope rules or by calling it manually.
    /// </summary>
    ...
    {
        using var nameStr = GetCString(zoneName, out ulong nameLen);
        using var fileStr = GetCString(filePath, out ulong fileLen);
        using var memberStr = GetCString(memberName, out ulong memberLen);

        ulong srcLocId = TracyAllocSrclocName((uint)lineNumber, fileStr, fileLen, memberStr, memberLen, nameStr, nameLen);
        ...
        if (text != null) {
            using var textStr = GetCString(text, out var textLen);
            TracyEmitZoneText(context, textStr, textLen);
        }
    ...
    private static CString GetCString(string? fromString, out ulong length)
    {
        if (fromString == null) {
            length = 0;
            return new CString(IntPtr.Zero);
        }

        length = (ulong)fromString.Length;
